...
REGEXES = {
    ...
    "divToPElementsRe": re.compile(r"<(a|blockquote|dl|div|ol|p|pre|table|ul)", re.I),
                ...
                if best_candidate:
                    ...
                    if ruthless:
                        ...
                        if article is None:
                            article = self.html
                print("In article \n\n")
                for img in article.cssselect("img"):
                    print("%s" % img.get("src"))
                ...
                if counts["p"] and counts["img"] > 1 + counts["p"] * 1.3:
                    ...
                    if el.tag == "div" and counts["img"] >= 1 and to_remove:
                        ...
                        for img in imgs:
                            ...
                            t_len = img.get("text_length")
                            log.debug(
                                "height %s text_length %s" % (repr(height), repr(t_len))
                            )
                            if to_int(height) >= 100 or to_int(t_len) >= 100:
                                valid_img = True
